...
class WindowCrossAttention3D(nn.Module):
    r"""Cross-Scale Window based multi-head self attention (CSW-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None):
        ...
        # define a parameter table of relative position bias
        self.relative_position_bias_table_x = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1) * (2 * window_size[2]),
                        num_heads))  # 2*Wd-1 * 2*Wh-1 * 2*Ww-1, nH
...
class SepSTSBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, depth_window_size=(1, 8, 8), shift_size=(0, 0, 0),
        ...
        self.depth_attn = WindowCrossAttention3D(
            dim, window_size=self.depth_window_size, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale)
    ...
    def forward_part1(self, x, mask_matrix1, mask_matrix2):
        B, D, H, W, C = x.shape
        ...
        if any(i > 0 for i in shift_size):
            ...
            attn_mask_x = mask_matrix1
            attn_mask_y = mask_matrix2
        else:
            shifted_x = x
            attn_mask_x = None
            attn_mask_y = None
        ...
        shifted_x_down = F.interpolate(shifted_x.permute(0, 4, 1, 2, 3).contiguous().view(-1, Dp, Hp, Wp), scale_factor=0.5, mode="bilinear", align_corners=False)
        shifted_x_down = F.pad(shifted_x_down, (window_size[1] // 4, window_size[1] // 4, window_size[2] // 4, window_size[2] // 4), mode='reflect')
        x_windows_down = F.unfold(shifted_x_down, kernel_size=window_size[1], dilation=1, padding=0, stride=window_size[1] // 2)
        x_windows_down = x_windows_down.view(B, C, window_size[0] * window_size[1] * window_size[2], -1)
        x_windows_down = x_windows_down.permute(0, 3, 2, 1).contiguous().view(-1, window_size[0] * window_size[1] * window_size[2], C)  # B*nW, Wd*Wh*Ww, C

        # CSWA/SW-CSWA
        attn_windows = self.depth_attn(x_windows, x_windows_down, mask_x=attn_mask_x, mask_y=attn_mask_y)  # B*nW, Wd*Wh*Ww, C
    ...
    def forward(self, x, mask_matrix1, mask_matrix2):
        """ Forward function.
        Args:
        ...
        shortcut = x
        x = self.forward_part1(x, mask_matrix1, mask_matrix2)
        x = shortcut + x
        x = x + self.forward_part2(x)

        return x

# compute origin feature map's shift-window mask
...
# compute the downscale feature map's shift-window mask
@lru_cache()
def compute_mask2(D, H, W, window_size, shift_size, device):
    ...
    ## downscale
    img_mask_down = F.interpolate(img_mask.permute(0, 4, 1, 2, 3).contiguous().view(-1, D, H, W), scale_factor=0.5, mode="bilinear", align_corners=False)
    img_mask_down = F.pad(img_mask_down, (window_size[1]//4, window_size[1]//4, window_size[2]//4, window_size[2]//4), mode='reflect')
    mask_windows_down = F.unfold(img_mask_down, kernel_size=window_size[1], dilation=1, padding=0, stride=window_size[1]//2)
    mask_windows_down = mask_windows_down.view(window_size[0]*window_size[1]*window_size[2], -1).permute(1, 0).contiguous()  # nW, ws[0]*ws[1]*ws[2]

    attn_mask = mask_windows_down.unsqueeze(1) - mask_windows.unsqueeze(2)  # nW, ws[0]*ws[1]*ws[2], ws[0]*ws[1]*ws[2]
    attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
...
class SepSTSBasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_mask2 = compute_mask2(Dp, Hp, Wp, window_size, shift_size, x.device)
        # print('attn_mask2: ', attn_mask2)
        # print(f'attn_mask.shape:{attn_mask.shape}\n'
        #       f'attn_mask2.shape:{attn_mask2.shape}\n')
        for i, blk in enumerate(self.blocks):
            x = blk(x, attn_mask, attn_mask2)
        x = x.view(B, D, H, W, -1)
